...
{
    public class Td_Ps_Rk_1BLL:BaseBLL
    {
        ...
        public override PageNavigate GetPage(Hashtable param = null)
        {
            ...
            var totalCount = DAL.GetCount(typeof(Td_Ps_Rk_1), param);
            if (totalCount > 0)
            {
                pn.Data = DAL.QueryPage<Td_Ps_Rk_1_Query>(typeof(Td_Ps_Rk_1), param);
        ...
        public override BaseResult Add(dynamic entity)
        {
            ...
            if (DAL.GetCount(typeof(Td_Ps_Rk_1), _ck_parm) > 0)
            {
                res.Success = false;
            ...
            //} 
            #endregion
            Td_Ps_Rk_1 psRk1 = new Td_Ps_Rk_1()
            {
                #region 赋值
                ...
                bm_djlx = Enums.Ps.PS130.ToString(),
            ...
            model.id = psRk1.id;
            //psRk1.bz = psRk1.bz.Trim();
            List<Td_Ps_Rk_2> psRk2S = new List<Td_Ps_Rk_2>();
            if (!string.IsNullOrEmpty(model.json_data))
            {
                psRk2S = JSON.Deserialize<List<Td_Ps_Rk_2>>(model.json_data);
                if (psRk2S.Any())
                {
                    int sort = 1;
                    psRk2S.ForEach(a =>
                    {
                        a.id = GetGuid;
                        a.id_masteruser = psRk1.id_masteruser;
                        a.id_bill = psRk1.id;
            ...
            var id_sps = (from p in psRk2S select p.id_sp).ToArray();
            var id_shops = new string[] { model.id_shop_ck, model.id_shop };
            ...
            if (model.AutoAudit)
            {
                ...
                Sh<Td_Ps_Rk_1>(res, param, "p_ps_rk_sh");
            }
            return res;
        }
        private bool CheckModel2(BaseResult res, Td_Ps_Rk_2 model)
        {
            if (model == null)
            {
                res.Success = false;
        ...
        public override BaseResult Update(dynamic entity)
        {
            ...
            DAL.UpdatePart(typeof(Td_Ps_Rk_1), param);
            param.Clear();
            param.Add("id_bill", model.id);
            param.Add("id_masteruser", model.id_masteruser);
            DAL.Delete(typeof(Td_Ps_Rk_2), param);
            DAL.AddRange(psRk2S);
        ...
        {
            BaseResult res = new BaseResult() { Success = true };
            Sh<Td_Ps_Rk_1>(res, param, "p_ps_rk_zf");
            return res;
        }
        ...
        public override BaseResult Delete(Hashtable param)
        {
            ...
            param.Add("id", id);
            var model = DAL.GetItem<Td_Ps_Rk_1>(typeof(Td_Ps_Rk_1), param);
            if (model == null)
            {
                res.Success = false;
                res.Message.Add("参数有误!");
            ...
            if (DAL.UpdatePart(typeof(Td_Ps_Rk_1), param) <= 0)
            {
                res.Success = false;
        ...
        {
            BaseResult res = new BaseResult() { Success = true };
            var psrk1 = DAL.GetItem<Td_Ps_Rk_1>(typeof(Td_Ps_Rk_1), param);
            PsrkQueryModel model = new PsrkQueryModel();
            if (psrk1 != null)
            {
                param.Clear();
                param.Add("id_bill", psrk1.id);
                param.Add("sort", "  sort_id  ");
                model.PsRk2List = DAL.QueryList<Td_Ps_Rk_2_Query>(typeof(Td_Ps_Rk_2), param).ToList();
                if (!string.IsNullOrEmpty(psrk1.id_create))
                {
                    param.Clear();
                    List<string> arr = new List<string>();
                    arr.Add(psrk1.id_create);
                    if (!string.IsNullOrEmpty(psrk1.id_user_sh))
                    {
                        arr.Add(psrk1.id_user_sh);
                    }
                    param.Add("id_users", arr.ToArray());
                    var users = DAL.QueryList<Tb_User>(typeof(Tb_User), param).ToList();
                    if (users.Any())
                    {
                        var user_zdr = users.FirstOrDefault(a => a.id == psrk1.id_create);
                        if (user_zdr != null) model.zdr = user_zdr.username;
                        if (!string.IsNullOrEmpty(psrk1.id_user_sh))
                        {
                            var user_shr = users.FirstOrDefault(a => a.id == psrk1.id_user_sh);
