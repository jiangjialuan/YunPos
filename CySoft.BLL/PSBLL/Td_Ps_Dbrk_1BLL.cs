...
{
    public class Td_Ps_Dbrk_1BLL : BaseBLL
    {
        ...
        public override PageNavigate GetPage(Hashtable param = null)
        {
            ...
            var totalCount = DAL.GetCount(typeof(Td_Ps_Dbrk_1), param);
            if (totalCount > 0)
            {
                pn.Data = DAL.QueryPage<Td_Ps_Dbrk_1_Query>(typeof(Td_Ps_Dbrk_1), param);
        ...
        public override BaseResult Add(dynamic entity)
        {
            ...
            if (DAL.GetCount(typeof(Td_Ps_Dbrk_1), _ck_parm) > 0)
            {
                res.Success = false;
                res.Message.Add("单号已存在!");
                return res;
            }
            if (string.IsNullOrEmpty(model.dh_origin)
                || string.IsNullOrEmpty(model.bm_djlx_origin)
                || string.IsNullOrEmpty(model.id_bill_origin))
            {
                res.Success = false;
                res.Message.Add("请选择调拨出库单!");
                return res;
            }
            ...
            #endregion
            Td_Ps_Dbrk_1 dbrk1 = new Td_Ps_Dbrk_1()
            {
                #region 赋值
                ...
                bm_djlx = Enums.Ps.PS320.ToString(),
            ...
            model.id = dbrk1.id;
            //dbrk1.bz = dbrk1.bz.Trim();
            List<Td_Ps_Dbrk_2> dbrk2S = new List<Td_Ps_Dbrk_2>();
            if (!string.IsNullOrEmpty(model.json_data))
            {
                dbrk2S = JSON.Deserialize<List<Td_Ps_Dbrk_2>>(model.json_data);
                if (dbrk2S.Any())
                {
                    int sort = 1;
                    dbrk2S.ForEach(a =>
                    {
                        a.id = GetGuid;
                        a.id_masteruser = dbrk1.id_masteruser;
                        a.id_bill = dbrk1.id;
            ...
            var id_sps = (from p in dbrk2S select p.id_sp).ToArray();
            var id_shops = new string[] { model.id_shop, model.id_shop_ck };
            ...
            if (model.AutoAudit)
            {
                ...
                Sh<Td_Ps_Dbrk_1>(res, param, "p_ps_dbrk_sh");
            }
            return res;
        }
        private bool CheckModel2(BaseResult res, Td_Ps_Dbrk_2 model)
        {
            if (model == null)
            {
                res.Success = false;
        ...
        public override BaseResult Update(dynamic entity)
        {
            ...
            DAL.UpdatePart(typeof(Td_Ps_Dbrk_1), param);
            param.Clear();
            param.Add("id_bill", model.id);
            param.Add("id_masteruser", model.id_masteruser);
            DAL.Delete(typeof(Td_Ps_Dbrk_2), param);
            DAL.AddRange(dbrk2S);
            return res;
        }
        ...
        {
            BaseResult res = new BaseResult() { Success = true };
            Sh<Td_Ps_Dbrk_1>(res, param, "p_ps_dbrk_zf");
            return res;
        }
        /// <summary>
        /// 审核
        ...
        public override BaseResult Delete(Hashtable param)
        {
            ...
            param.Add("id", id);
            var model = DAL.GetItem<Td_Ps_Dbrk_1>(typeof(Td_Ps_Dbrk_1), param);
            if (model == null)
            {
                res.Success = false;
                res.Message.Add("参数有误!");
            ...
            if (DAL.UpdatePart(typeof(Td_Ps_Dbrk_1), param) <= 0)
            {
                res.Success = false;
        ...
        {
            BaseResult res = new BaseResult() { Success = true };
            var dbrk1 = DAL.GetItem<Td_Ps_Dbrk_1>(typeof(Td_Ps_Dbrk_1), param);
            DbrkQueryModel model = new DbrkQueryModel();
            if (dbrk1 != null)
            {
                param.Clear();
                param.Add("id_bill", dbrk1.id);
                param.Add("sort", "  sort_id  ");
                model.dbrk2List = DAL.QueryList<Td_Ps_Dbrk_2_Query>(typeof(Td_Ps_Dbrk_2), param).ToList();
                if (!string.IsNullOrEmpty(dbrk1.id_create))
                {
                    param.Clear();
                    List<string> arr = new List<string>();
                    arr.Add(dbrk1.id_create);
                    if (!string.IsNullOrEmpty(dbrk1.id_user_sh))
                    {
                        arr.Add(dbrk1.id_user_sh);
                    }
                    param.Add("id_users", arr.ToArray());
                    var users = DAL.QueryList<Tb_User>(typeof(Tb_User), param).ToList();
                    if (users.Any())
                    {
                        var user_zdr = users.FirstOrDefault(a => a.id == dbrk1.id_create);
                        if (user_zdr != null) model.zdr = user_zdr.username;
                        if (!string.IsNullOrEmpty(dbrk1.id_user_sh))
                        {
                            var user_shr = users.FirstOrDefault(a => a.id == dbrk1.id_user_sh);
