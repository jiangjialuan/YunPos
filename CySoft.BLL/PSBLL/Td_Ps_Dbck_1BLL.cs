...
{
    public class Td_Ps_Dbck_1BLL : BaseBLL
    {
        #region 获取分页数据
        public override PageNavigate GetPage(Hashtable param = null)
        {
            PageNavigate pn = new PageNavigate() { Success = true };
            var totalCount = DAL.GetCount(typeof(Td_Ps_Dbck_1), param);
            if (totalCount > 0)
            {
                pn.Data = DAL.QueryPage<Td_Ps_Dbck_1_Query>(typeof(Td_Ps_Dbck_1), param);
        ...
        public override BaseResult Add(dynamic entity)
        {
            ...
            if (DAL.GetCount(typeof(Td_Ps_Dbck_1), _ck_parm) > 0)
            {
                res.Success = false;
            ...
            #endregion
            Td_Ps_Dbck_1 dbck1 = new Td_Ps_Dbck_1()
            {
                #region 赋值
                ...
                bm_djlx = Enums.Ps.PS310.ToString(),
            ...
            model.id = dbck1.id;
            //dbck1.bz = dbck1.bz.Trim();
            List<Td_Ps_Dbck_2> dbck2S = new List<Td_Ps_Dbck_2>();
            if (!string.IsNullOrEmpty(model.json_data))
            {
                dbck2S = JSON.Deserialize<List<Td_Ps_Dbck_2>>(model.json_data);
                if (dbck2S.Any())
                {
                    int sort = 1;
                    dbck2S.ForEach(a =>
                    {
                        a.id = GetGuid;
                        a.id_masteruser = dbck1.id_masteruser;
                        a.id_bill = dbck1.id;
            ...
            var id_sps = (from p in dbck2S select p.id_sp).ToArray();
            var id_shops = new string[] { dbck1.id_shop, dbck1.id_shop_rk };
            ht.Add("id_shop_array", id_shops);
            ht.Add("id_sp_array", id_sps);
            ht.Add("id_masteruser", dbck1.id_masteruser);
            ...
            if (model.AutoAudit)
            {
                ...
                Sh<Td_Ps_Dbck_1>(res, param, "p_ps_dbck_sh");
            }
            return res;
        }
        private bool CheckModel2(BaseResult res, Td_Ps_Dbck_2 model)
        {
            if (model == null)
            {
                res.Success = false;
        ...
        public override BaseResult Update(dynamic entity)
        {
            ...
            DAL.UpdatePart(typeof(Td_Ps_Dbck_1), param);
            param.Clear();
            param.Add("id_bill", model.id);
            param.Add("id_masteruser", model.id_masteruser);
            DAL.Delete(typeof(Td_Ps_Dbck_2), param);
            DAL.AddRange(dbck2S);
            return res;
        }
        ...
        {
            BaseResult res = new BaseResult() { Success = true };
            Sh<Td_Ps_Dbck_1>(res, param, "p_ps_dbck_zf");
            return res;
        }
        [Transaction]
        public override BaseResult Delete(Hashtable param)
        {
            ...
            param.Add("id", id);
            var model = DAL.GetItem<Td_Ps_Dbck_1>(typeof(Td_Ps_Dbck_1), param);
            if (model == null)
            {
                res.Success = false;
                res.Message.Add("参数有误!");
            ...
            if (DAL.UpdatePart(typeof(Td_Ps_Dbck_1), param) <= 0)
            {
                res.Success = false;
