...
{
    [LoginActionFilter]
    public class KcpdController:BaseController
    {
        [ActionPurview(true)]
        public ActionResult List()
        {
            ...
            try
            {
                ...
                pn = BusinessFactory.Td_Kc_Kspd_1.GetPage<Td_Kc_Kspd_1, Td_Kc_Kspd_1_Query>(param);

                var plist = new PageList<Td_Kc_Kspd_1_Query>(pn, pageIndex, limit);
        ...
        [HttpPost]
        [ActionPurview(true)]
        public ActionResult Add(KspdModel model)
        {
        ...
        public ActionResult Edit(string id)
        {
            ...
            param.Add("id",id);
            param.Add("id_masteruser",id_user_master);
            var model = BusinessFactory.Td_Kc_Kspd_1.Get(param).Data as KspdQueryModel;
            if (model!=null)
            {
        ...
        public ActionResult Copy(string id)
        {
            ...
            ViewData["item_edit"] = model;// BusinessFactory.Td_Kc_Kspd_1.Get(param).Data;
            return View("Edit");
        }
        ...
        public ActionResult Sh(string id)
        {
            var res = HandleResult(() =>
            {
                ...
                return BusinessFactory.Td_Kc_Kspd_1.Active(param);
            });
            return JsonString(res, 1);
        }
        /// <summary>
        ...
        public ActionResult ZF(string id)
        {
            var res = HandleResult(() =>
            {
                ...
                param.Add("id_masteruser", id_user_master);
                param.Add("id_user", id_user);
                return BusinessFactory.Td_Kc_Kspd_1.Stop(param);
            });
            return JsonString(res, 1);
        }
        ...
        public ActionResult Delete(string id)
        {
            var res = HandleResult(() =>
            {
                ...
                param.Add("id_masteruser", id_user_master);
                param.Add("id_user", id_user);
                return BusinessFactory.Td_Kc_Kspd_1.Delete(param);
            });
            return JsonString(res, 1);
        }
        ...
        public ActionResult ImportIn(string filePath)
        {
            ...
            List<KspdShopsp_Import> list = new List<KspdShopsp_Import>();
        ...
        private string SaveFailFile(List<KspdShopsp_Import> failList)
        {
            try
            {
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "盘点商品资料模版");
                int i = 1;
        ...
        private List<KspdShopsp_Import> TurnImportList(DataTable table)
        {
            List<KspdShopsp_Import> list = new List<KspdShopsp_Import>();
            foreach (DataRow item in table.Rows)
            {
                KspdShopsp_Import model = new KspdShopsp_Import();
                model.barcode =string.Format("{0}",item["商品条码"]);
                model.mc = string.Format("{0}", item["商品名称"]);
                model.dw = string.Format("{0}", item["单位"]);
                decimal sl = 0;
                decimal.TryParse(string.Format("{0}", item["实盘数量"]), out sl);
                model.sl = sl;
        ...
        private void ProccessData(string filePath, ref List<KspdShopsp_Import> list, ref List<KspdShopsp_Import> successList, ref List<KspdShopsp_Import> failList, string id_shop_sel = "")
        {
            ...
            if (!string.IsNullOrEmpty(id_shop_sel))
                ...
                param.Add("id_shop", id_shop);
            br = BusinessFactory.Td_Kc_Kspd_1.Export(param);
            if (br.Success)
            {
                KspdShopsp_Import_All rModel = (KspdShopsp_Import_All)br.Data;
        ...
        public FileResult DownloadExcelTemp()
        {
            try
            {
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "盘点商品资料模板");
                ...
                rowtemp.CreateCell(4).SetCellValue("盘点商品");
                ...
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
                return File(ms, "application/vnd.ms-excel", "盘点商品资料模板-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
        ...
        public FileResult ImportOut()
        {
            ...
            ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "盘点商品资料导出");
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            #endregion
            ...
            try
            {
                ...
                param = param.Trim(p);
                List<Td_Kc_Kspd_2_Query> shopspList = JSON.Deserialize<List<Td_Kc_Kspd_2_Query>>(param["shopspList"].ToString()) ?? new List<Td_Kc_Kspd_2_Query>();
                #endregion
                #region 验证数据
                if (shopspList == null || !shopspList.Any())
                {
                    ...
                    rowtemp.CreateCell(5).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(6).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(7).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(8).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(9).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(10).SetCellValue(Decimal.Round((decimal)(0), 2).ToString());
                    rowtemp.CreateCell(11).SetCellValue("");
                    sheet1.GetRow(1).Height = 28 * 20;
                    // 写入到客户端 
                    book.Write(ms);
                    ms.Seek(0, SeekOrigin.Begin);
                    return File(ms, "application/vnd.ms-excel", "盘点商品资料导出-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
                }
                #endregion
                #region 数量、单价、金额小数点
                var digit = GetParm();
                ...
                foreach (var item in shopspList)
                {
                    ...
                    rowtempdata.CreateCell(6).SetCellValue(Decimal.Round((decimal)(item.sl_yk), int.Parse(digit["sl_digit"].ToString())).ToString());
                    ...
                    rowtempdata.CreateCell(9).SetCellValue(Decimal.Round((decimal)(item.je_cb), int.Parse(digit["je_digit"].ToString())).ToString());
                    rowtempdata.CreateCell(10).SetCellValue(Decimal.Round((decimal)(item.je_yk), int.Parse(digit["je_digit"].ToString())).ToString());
            ...
            catch (Exception ex)
            {
                ...
                rowtempe.CreateCell(6).SetCellValue("盘点导出出现异常 请重试");
                sheet1.GetRow(1).Height = 28 * 20;
                // 写入到客户端 
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
