    ...
    public class ThController : BaseController
    {
        ...
        public ActionResult List()
        {
            ...
            try
            {
                ...
                pn = BusinessFactory.Td_Jh_Th_1.GetPage(param);
                var plist = new PageList<Td_Jh_Th_1_QueryModel>(pn, pageIndex, limit);
        ...
        public ActionResult Add()
        {
            ...
            {
                var br = BusinessFactory.Td_Jh_Th_1.Get(param);
                if (br.Success)
                {
            ...
            else if (param.ContainsKey("id_jh") && !string.IsNullOrEmpty(param["id_jh"].ToString()))
            {
                ...
                var br = BusinessFactory.Td_Jh_1.Get(ht);
                if (br.Success)
                {
                    ViewBag.CopyJhInfo = br.Data;
                }
            }
        ...
        public ActionResult Delete()
        {
            ...
            try
            {
                ...
                br = BusinessFactory.Td_Jh_Th_1.Delete(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("退货-删除", oldParam, br);
        ...
        public ActionResult Stop()
        {
            ...
            try
            {
                param_model = param.Trim(pv);
                oldParam = (Hashtable)param_model.Clone();
                br = BusinessFactory.Td_Jh_Th_1.Stop(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
        ...
        public ActionResult CreateDH()
        {
            ...
            {
                br.Success = true;
                br.Data = GetNewDH(Enums.FlagDJLX.DHTH);
        ...
        public FileResult ImportOut()
        {
            ...
            ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "退货商品资料导出");
            ...
            try
            {
                ...
                if (shopspList == null || shopspList.Count() <= 0)
                {
                    ...
                    return File(ms, "application/vnd.ms-excel", "退货商品资料导出-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
                }
                #endregion
                #region 数量、单价、金额小数点
                var digit = GetParm();
            ...
            catch (Exception ex)
            {
                ...
                rowtempe.CreateCell(6).SetCellValue("退货导出出现异常 请重试");
                sheet1.GetRow(1).Height = 28 * 20;
                // 写入到客户端 
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
        ...
        private string SaveFailFile(List<Tb_JhShopsp_Import> failList)
        {
            try
            {
                string fileName = "退货商品导入失败数据" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".xls";
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "退货商品资料模版");
                int i = 1;
        ...
        public FileResult DownloadExcelTemp()
        {
            try
            {
                ...
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
                return File(ms, "application/vnd.ms-excel", "退货商品资料模板-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
