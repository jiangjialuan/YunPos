...
{
    [LoginActionFilter]
    public class XsckController : BaseController
    {
        ...
        public ActionResult List()
        {
            ...
            try
            {
                ...
                pn = BusinessFactory.Td_Xs_1.GetPage(param);
                var plist = new PageList<Td_Xs_1_QueryModel>(pn, pageIndex, limit);
        ...
        public ActionResult Add()
        {
            ...
            p.Add("id_dd", string.Empty, HandleType.Remove);//是否销售单引单
            param = param.Trim(p);
            ...
            {
                var br = BusinessFactory.Td_Xs_1.Get(param);
                if (br.Success)
                {
                    ViewBag.CopyInfo = br.Data;
                }
            }
            else if (param.ContainsKey("id_dd") && !string.IsNullOrEmpty(param["id_dd"].ToString()))
            {
                Hashtable ht = new Hashtable();
                ht.Add("id", param["id_dd"].ToString());
                var br = BusinessFactory.Td_Xs_Dd_1.Get(ht);
                if (br.Success)
                {
                    ViewBag.CopyDDInfo = br.Data;
                }
            }
        ...
        public ActionResult CreateDH()
        {
            ...
            {
                br.Success = true;
                br.Data = GetNewDH(Enums.FlagDJLX.XSCK);
        ...
        public ActionResult Stop()
        {
            ...
            try
            {
                ...
                br = BusinessFactory.Td_Xs_1.Stop(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("销售出库-作废", oldParam, br);
        ...
        public ActionResult Delete()
        {
            ...
            try
            {
                param_model = param.Trim(pv);
                oldParam = (Hashtable)param_model.Clone();
                br = BusinessFactory.Td_Xs_1.Delete(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("销售出库-删除", oldParam, br);
            return JsonString(br, 1);
        }
        #endregion

        #region 销售出库-批量导入商品 UI
        /// <summary>
        /// 销售出库-批量导入商品 UI 
        /// lz
        /// 2016-09-26
        ...
        private string SaveFailFile(List<Tb_JhShopsp_Import> failList)
        {
            try
            {
                string fileName = "销售出库商品导入失败数据" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".xls";
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "销售出库商品资料模版");
                int i = 1;
        ...
        public FileResult DownloadExcelTemp()
        {
            try
            {
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "销售出库商品资料模板");
                ...
                rowtemp.CreateCell(4).SetCellValue("2016-09-24销售出库商品");
                ...
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
                return File(ms, "application/vnd.ms-excel", "销售出库商品资料模板-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
        ...
        public FileResult ImportOut()
        {
            ...
            ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "销售出库商品资料导出");
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            #endregion
            ...
            try
            {
                ...
                if (shopspList == null || shopspList.Count() <= 0)
                {
                    ...
                    book.Write(ms);
                    ms.Seek(0, SeekOrigin.Begin);
                    return File(ms, "application/vnd.ms-excel", "销售出库商品资料导出-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
                }
                #endregion
                #region 数量、单价、金额小数点
                var digit = GetParm();
            ...
            catch (Exception ex)
            {
                ...
                rowtempe.CreateCell(6).SetCellValue("销售出库导出出现异常 请重试");
                sheet1.GetRow(1).Height = 28 * 20;
                // 写入到客户端 
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
        ...
        public ActionResult Search()
        {
            ...
            try
            {
                ...
                pv.Add("jh_callback", "", HandleType.Remove);
                ...
                if (param.ContainsKey("jh_callback") && !string.IsNullOrEmpty(param["jh_callback"].ToString()))
                    ViewData["jh_callback"] = param["jh_callback"].ToString();
        ...
        #region 根据id获取详细
        [ActionPurview(false)]
        public ActionResult GetCkDetailList()
        {
            ...
            try
            {
                ...
                if (param.ContainsKey("id") && !string.IsNullOrEmpty(param["id"].ToString()))
                {
                    br = BusinessFactory.Td_Xs_1.Get(param);
                }
                return JsonString(br);
            }
            catch (Exception ex)
        ...
        public ActionResult Sh()
        {
            ...
            try
            {
                ...
                param.Add("id_user", id_user);
                oldParam = (Hashtable)param.Clone();
                br = BusinessFactory.Td_Xs_1.Active(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("销售出库-审核", oldParam, br);
            return JsonString(br, 1);
        }
        #endregion
        ...
        public ActionResult AllowTH()
        {
            ...
            {
                param_model = param.Trim(pv);
                br = BusinessFactory.Td_Xs_1.Export(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
