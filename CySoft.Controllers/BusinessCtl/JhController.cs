...
namespace CySoft.Controllers.BusinessCtl
{
    ...
    public class JhController : BaseController
    {
        ...
        public ActionResult Add()
        {
            ...
            p.Add("id_dh", string.Empty, HandleType.Remove);//是否订货引单
            param = param.Trim(p);
            ...
            else if (param.ContainsKey("id_dh") && !string.IsNullOrEmpty(param["id_dh"].ToString()))
            {
                Hashtable ht = new Hashtable();
                ht.Add("id", param["id_dh"].ToString());
                var br = BusinessFactory.Td_Jh_Dd_1.Get(ht);
                if (br.Success)
                {
                    ViewBag.CopyDhInfo = br.Data;
                }
            }
        ...
        public ActionResult Add(Tb_Shopsp model)
        {
            ...
            try
            {
                ...
                List<Td_Jh_2> shopspList = JSON.Deserialize<List<Td_Jh_2>>(param["shopspList"].ToString()) ?? new List<Td_Jh_2>();
                ...
                foreach (var item in shopspList)
                {
                    ...
                    else
                    {
                        ...
                        WriteDBLog("进货-新增/更新", oldParam, br);
                        return base.JsonString(new
                        {
                            status = "error",
                            message = string.Join(";", br.Message)
                ...
                {
                    br = BusinessFactory.Td_Jh_1.Update(param);
                    WriteDBLog("进货-更新", oldParam, br);
                }
                else
                {
                    //插入表
                    br = BusinessFactory.Td_Jh_1.Add(param);
                    WriteDBLog("进货-新增", oldParam, br);
                }
        ...
        public ActionResult Stop()
        {
            ...
            try
            {
                ...
                br = BusinessFactory.Td_Jh_1.Stop(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("进货-作废", oldParam, br);
        ...
        public ActionResult CreateDH()
        {
            ...
            {
                br.Success = true;
                br.Data = GetNewDH(Enums.FlagDJLX.DHJH);
        ...
        private string SaveFailFile(List<Tb_JhShopsp_Import> failList)
        {
            try
            {
                string fileName = "进货商品导入失败数据" + DateTime.Now.ToString("yyyyMMddHHmmssfff") + ".xls";
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "进货商品资料模版");
                int i = 1;
        ...
        public FileResult DownloadExcelTemp()
        {
            try
            {
                ...
                ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "收货商品资料模板");
                ...
                rowtemp.CreateCell(4).SetCellValue("2016-09-24进货商品");
                ...
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
                return File(ms, "application/vnd.ms-excel", "收货商品资料模板-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
        ...
        public FileResult ImportOut()
        {
            ...
            ISheet sheet1 = NPOIHelper.CreateSheet(book, e_param, e_param_comment, "收货商品资料导出");
            System.IO.MemoryStream ms = new System.IO.MemoryStream();
            #endregion
            ...
            try
            {
                ...
                if (shopspList == null || shopspList.Count() <= 0)
                {
                    ...
                    book.Write(ms);
                    ms.Seek(0, SeekOrigin.Begin);
                    return File(ms, "application/vnd.ms-excel", "收货商品资料导出-" + DateTime.Now.ToString("yyyyMMddhhmmss") + ".xls");
                }
                #endregion
                #region 数量、单价、金额小数点
                var digit = GetParm();
            ...
            catch (Exception ex)
            {
                ...
                rowtempe.CreateCell(6).SetCellValue("收货导出出现异常 请重试");
                sheet1.GetRow(1).Height = 28 * 20;
                // 写入到客户端 
                book.Write(ms);
                ms.Seek(0, SeekOrigin.Begin);
        ...
        #region 根据id获取详细
        [ActionPurview(false)]
        public ActionResult GetJhDetailList()
        {
            ...
            try
            {
                ...
                if (param.ContainsKey("id") && !string.IsNullOrEmpty(param["id"].ToString()))
                {
                    br = BusinessFactory.Td_Jh_1.Get(param);
                }
                return JsonString(br);
            }
            catch (Exception ex)
        ...
        public ActionResult Sh()
        {
            ...
            try
            {
                ...
                param.Add("id_user", id_user);
                oldParam = (Hashtable)param.Clone();
                br = BusinessFactory.Td_Jh_1.Active(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
            }
            WriteDBLog("进货-审核", oldParam, br);
            return JsonString(br, 1);
        }
        #endregion
        ...
        public ActionResult AllowTH()
        {
            ...
            {
                param_model = param.Trim(pv);
                br = BusinessFactory.Td_Jh_1.Export(param);
            }
            catch (Exception ex)
            {
                br.Message.Add(ex.Message);
