...
$(function () {
    ...
    $(function () {
        ...
        left = (width - $('.dialog_box_').width()) / 2;
        top = (height - $('.dialog_box_').height()) / 2;
        if (top <= parseFloat($('.dialog_box_').height() + 200)) {
            $('.dialog_box_').css({
                left: left,
                top: top
            });
    ...
    $('.login-btn').on('click', function () {
        ...
        if (!userName1) {
            $username1.focus();
            $warnArea1.html('用户名不能为空').removeClass('hide');
        }
        else if (!userPassword1) {
            $password1.focus();
            $warnArea1.html('密码不能为空').removeClass('hide');
        } 
        else if (!recognitionReg.test(img_code1) && loginCount >= 3) {
            $img_code1.focus();
            $warnArea1.html('请输入正确的验证码').removeClass('hide');
        }
        else {
            var smData = {};            
                    ...
                    if (ret && ret.Success) {
                        ...
                        if (ret.Level == '2')
                        {
                            ...
                            if ($('.loginItem .active').index() == 0) {
                                ...
                                $warnArea1.html(msg).removeClass('hide');
                            } else {
                                $('#form2 .btn_img_code').click();
                                $('#form2 .recognition-code').val('');
                                loginCount = loginCount + 1;
                                if (loginCount >= 3) {
                                    $("#form2 #div_img_code").show();
                                    $("#form2 #div_img_code img").click();
                                }
                                var msg = '';
                                $.each(ret.Message, function (i, obj) {
                                    msg += obj;
                                ...
                                $warnArea2.html(msg).removeClass('hide');
                            }
    ...
    $username1.keypress(function (e) {
        debugger;
        $warnArea1.html('').removeClass('hide');
        e = e || event;
    ...
    $qynumber.keypress(function (e) {
        debugger;
        $warnArea2.html('').removeClass('hide');
        e = e || event;
        if (e.charCode === 13 || e.keyCode === 13) {
            //$('.login-btn').trigger('click');
            $username2.focus();
